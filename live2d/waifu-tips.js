...
live2d_settings['homePageUrl'] = 'https://github.com/su-xiaoyu' // 主页地址，可选 'auto'(自动), '{URL 网址}'
		...
		$.ajax({
			...
			url:
				waifuPath == ''
					...
					: waifuPath.substr(waifuPath.length - 15) ==
					  'waifu-tips.json'
					? waifuPath
			...
			if (document.referrer !== '') {
				...
				if (window.location.hostname == referrer.hostname)
					...
					$.each(result.waifu.referrer_hostname, function (i, val) {
						if (i == referrer.hostname)
							referrer.hostname = getRandText(val)
					})
					text =
				...
				$.getJSON(
					...
					function (result) {
						if (!empty(result.source)) {
							var text =
								waifu_tips.hitokoto_api_message['lwl12.com'][0]
							if (!empty(result.author))
								text +=
									waifu_tips.hitokoto_api_message[
										'lwl12.com'
									][1]
							text = text.render({
			...
			case 'fghrsh.net':
				$.getJSON(
					...
					function (result) {
						if (!empty(result.source)) {
							var text =
								waifu_tips.hitokoto_api_message['fghrsh.net'][0]
							text = text.render({
								source: result.source,
								date: result.date,
							})
			...
			default:
				$.getJSON('https://v1.hitokoto.cn', function (result) {
					if (!empty(result.from)) {
						var text =
							waifu_tips.hitokoto_api_message['hitokoto.cn'][0]
						text = text.render({
							source: result.from,
